...
`AgentLoop` 实现的是"服务端自主控制"的工具型循环，核心是**多轮工具调用直到 LLM 直接回答**。
...
| 工具名 | 实现类 | 功能 |
|--------|--------|------|
| `search_documents` | `SearchDocumentsTool` | 在文档标题中模糊搜索，返回有权限查看的文档列表 |
| `list_documents` | `ListDocumentsTool` | 获取当前用户有权访问的文档列表 |
| `get_document_detail` | `GetDocumentDetailTool` | 获取指定文档的详细信息（标题、摘要、内容片段等） |
| `list_knowledge_bases` | `ListKnowledgeBasesTool` | 获取当前用户有权访问的知识库列表 |
| `rag_qa` | `RagQaTool` | 基于 Milvus 向量检索回答用户问题，返回最相关的文档及匹配片段 |
...
说明：`RagQaTool` 现在提供的是"检索结果与证据片段"，而不是在 Tool 内部直接拼装最终自然语言答案。
...
### 4.5 Controller 逐接口说明

#### 4.5.1 KbDocumentController（文档管理）

| 接口 | 方法 | 作用 |
|------|------|------|
| `GET /api/kb/documents` | 分页查询 | 按用户可见范围查文档 |
| `GET /api/kb/documents/{id}` | 详情查询 | 单文档查询 + 权限校验 |
| `POST /api/kb/documents/upload` | 上传 | 文档先入库 PENDING，不会立即分块 |
| `POST /api/kb/documents/{id}/start-chunk` | 提交分块 | 异步分块任务，文档需为 PENDING/FAILED |
| `POST /api/kb/documents/{id}/execute-chunk` | 立即分块 | 偏运维的补偿接口 |
| `PUT /api/kb/documents/{id}` | 更新 | 更新标题、配置，不重新分块 |
| `PATCH /api/kb/documents/{id}/enabled` | 启停 | 联动 chunk enabled 与向量状态 |
| `GET /api/kb/documents/{id}/chunk-logs` | 分块日志 | 用于诊断成功/失败链路 |
| `GET /api/kb/documents/search` | 标题搜索 | 按标题模糊搜文档 |
| `GET /api/kb/documents/{id}/download` | 下载 | 校验可见性后读文件流 |
| `DELETE /api/kb/documents/{id}` | 删除 | 先删向量，再删 chunk、权限、文档 |

#### 4.5.2 KbKnowledgeBaseController（知识库）

| 接口 | 方法 | 作用 |
|------|------|------|
| `POST /api/kb/bases` | 创建 | 同时确保 Milvus collection 可用 |
| `PUT /api/kb/bases/{id}` | 更新 | 不允许在已有向量化文档时切换 embedding model |
| `PUT /api/kb/bases/{id}/rename` | 重命名 | 只改名称，更轻量 |
| `DELETE /api/kb/bases/{id}` | 删除 | 前提：库下不能还有文档 |
| `GET /api/kb/bases/{id}` | 详情 | 附带文档数量统计 |
| `GET /api/kb/bases` | 分页 | 非管理员只能看自己名下的 |

#### 4.5.3 KbCategoryController（分类）

当前只有 CRUD 接口，业务逻辑轻，直接通过 `ServiceImpl` 基础能力完成。

#### 4.5.4 KbChunkController（Chunk 维护）

| 接口 | 方法 | 作用 |
|------|------|------|
| `GET /api/kb/documents/{docId}/chunks` | 分页查询 | 可按 enabled 过滤 |
| `GET /api/kb/documents/{docId}/chunks/list` | 全量查询 | 适合管理页面展示 |
| `POST /api/kb/documents/{docId}/chunks` | 手工新增 | 新增时即时写向量 |
| `POST /api/kb/documents/{docId}/chunks/batch` | 批量新增 | 可控制是否当场写向量 |
| `PUT /api/kb/documents/{docId}/chunks/{chunkId}` | 更新 | 同步更新向量 |
| `DELETE /api/kb/documents/{docId}/chunks/{chunkId}` | 删除 | 先删向量再删 chunk |
| `PATCH /api/kb/documents/{docId}/chunks/{chunkId}/enabled` | 启停 | 启用补写向量，禁用删向量 |
| `POST /api/kb/documents/{docId}/chunks/batch-enabled` | 批量启停 | 面向管理页面 |
...
| 顺序 | 方法 | 说明 |
|------|------|------|
| 1 | `KbDocumentController.upload(...)` | HTTP 入口 |
| 2 | `KbDocumentServiceImpl.upload(...)` | 门面编排 |
| 3 | `DocumentUploadService.upload(...)` | 核心上传逻辑 |
| 4 | `DocumentUploadService.savePermissionRows(...)` | 权限行写入 |

#### 6.7.2 文档分块

| 顺序 | 方法 | 说明 |
|------|------|------|
| 1 | `KbDocumentController.startChunk(...)` | HTTP 入口 |
| 2 | `KbDocumentServiceImpl.startChunk(...)` | 门面编排 |
| 3 | `DocumentChunkingService.startChunk(...)` | 任务投递（CAS + 事件发布） |
| 4 | `DocumentChunkEventListener.onChunkRequested(...)` | 异步监听 |
| 5 | `DocumentChunkingService.executeChunk(...)` | 执行入口 |
| 6 | `DocumentChunkingService.runChunkTask(...)` | 核心分块任务 |
| 7 | `DocumentChunkingService.persistChunksAndVectorsAtomically(...)` | 原子持久化 |

#### 6.7.3 文档删除与启停

| 顺序 | 方法 | 说明 |
|------|------|------|
| 1 | `KbDocumentServiceImpl.enableDocument(...)` | 启停文档 |
| 2 | `KbDocumentServiceImpl.deleteVisible(...)` | 删除文档 |
| 3 | `DocumentDeleteService.deleteDocument(...)` | 删除实现 |


### 7.9 Service 主方法逐段解释

#### 7.9.1 DocumentChunkingService.startChunk(...)
...
#### 7.9.2 DocumentChunkingService.runChunkTask(...)

这是最值得反复看的方法之一。

**主步骤**：
1. 初始化 chunk log
2. 解析 `ProcessMode`（当前 PIPELINE 还没接入）
3. 读取本地文件流
4. 调 `TikaDocumentParser.extractWithMetadata(...)`
5. 根据 `chunkStrategy` 找到具体 `ChunkingStrategy`
6. 用 `ChunkingOptions` 生成分块参数
7. 执行分块，得到 `List<TextChunk>`
8. 判断是否要向量化
9. 如需向量化则批量 embedding
10. 为每个 chunk 生成主键、组装 `VectorDocChunk`
11. 调 `persistChunksAndVectorsAtomically(...)`
12. 更新分块日志为成功

**异常处理**：记录异常、`markChunkFailed(...)`、更新分块日志为失败

#### 7.9.3 KbChunkServiceImpl.create(...)

人工维护 chunk 的典型动作：
1. 加载文档、校验读写权限、校验文档不在 `RUNNING`
2. 校验内容非空
3. 决定 `chunkIndex`
4. 组装 `KbDocumentChunk`
5. 调 `vectorSyncService.syncChunk(...)`
6. 插入 chunk，回写文档 `chunkCount`

说明：当前手工新增 chunk 默认希望它立即进入检索体系。

#### 7.9.4 KbChunkServiceImpl.update(...)
...
#### 7.9.5 KbKnowledgeBaseServiceImpl.create(...)

知识库创建的关键点不在插表，而在"Milvus collection 同步创建"：
1. 校验知识库名、collection 名合法性
2. 校验不重名
3. 插入 `kb_knowledge_base`
4. 调 `milvusCollectionHelper.ensureCollectionLoaded(collection)`

知识库不是纯逻辑概念，它和 Milvus collection 是显式绑定关系。
